...
{
	ostr += "{ ";
    map<string,JsonValuePtr>::const_iterator it(p->value.begin()), it_end(p->value.end());
	while (it != it_end)
    {
...
//////////////////////////////////////////////////////
void TC_JsonWriteOstram::writeValue(const JsonValuePtr & p, ostream& ostr)
{
    if(!p)
    {
    	ostr << "null";
...
void TC_JsonWriteOstram::writeString(const JsonValueStringPtr & p, ostream& sReturn)
{
	writeString(p->value, sReturn);
}

void TC_JsonWriteOstram::writeString(const string & s, ostream& sReturn)
{
    sReturn << "\"";
    std::string::const_iterator it(s.begin()),
                                itEnd(s.end());
...
void TC_JsonWriteOstram::writeNum(const JsonValueNumPtr & p, ostream& ostr)
{
    if(p->isInt)
    	ostr<<setprecision(30)<<p->value;
    else
    	ostr<<p->value;
}


void TC_JsonWriteOstram::writeObj(const JsonValueObjPtr & p, ostream& ostr)
{
	ostr << "{ ";
    map<string,JsonValuePtr>::const_iterator it(p->value.begin()), it_end(p->value.end());
	while (it != it_end)
...
void TC_JsonWriteOstram::writeArray(const JsonValueArrayPtr & p, ostream& ostr)
{
	ostr << "[ ";
    vector<JsonValuePtr>::const_iterator it(p->value.begin()), it_end(p->value.end());
	while (it != it_end)
...
void TC_JsonWriteOstram::writeBoolean(const JsonValueBooleanPtr & p, ostream& ostr)
{
    if(p->value)
    	ostr << "true";
    else
